...
class AuthServices {
  Future<void> signUser(BuildContext context,
    ...
    if (response.statusCode == 200) {
      ...
      showSnackBar('Connexion effectuée avec success', context, Colors.green);
    } else {
      //if (response.statusCode == 404) {
      showSnackBar(
          'Ces identifiants ne sont pas reconnus', context, Colors.red);
    } // else {
    //showSnackBar('Une erreur est survenue', context, Colors.red);
    //}
  }
  ...
  Future<void> signUp(RegisterUser user, BuildContext context) async {
    ...
    if (response.statusCode == 200) {
      showSnackBar('Votre compte a été avec success', context, Colors.green);
    } else {
      showSnackBar('Une erreur du serveur', context, Colors.red);
    }
  }
}
